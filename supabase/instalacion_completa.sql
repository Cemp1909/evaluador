-- USAR SOLO en un proyecto Supabase nuevo y vacío.
-- Crea las 12 tablas iniciales y aplica todos los permisos y campos.
-- Ejecutar completo en SQL Editor de Supabase.

begin;

-- Base inicial para la información académica. No incluye contraseñas.
-- Todas las tablas tienen RLS y no se abren políticas hasta implementar
...
create unique index colegios_nombre_unico
  on public.colegios (lower(btrim(nombre)));

create table public.docentes_colegio (
  id uuid primary key default gen_random_uuid(),
  nombre text not null check (length(btrim(nombre)) > 0),
  creado_en timestamptz not null default now()
);

-- Cada cambio de colegio o nivel abre una asignación nueva y conserva el historial.
create table public.asignaciones_docentes (
  id uuid primary key default gen_random_uuid(),
  docente_id uuid not null references public.docentes_colegio(id),
  colegio_id uuid not null references public.colegios(id),
  nivel text not null check (nivel in ('preescolar', 'primaria')),
  fecha_inicio timestamptz not null,
  fecha_fin timestamptz,
  check (fecha_fin is null or fecha_fin > fecha_inicio)
);
create unique index asignacion_docente_activa_unica
  on public.asignaciones_docentes (docente_id) where fecha_fin is null;
create index asignaciones_docentes_colegio_nivel
  on public.asignaciones_docentes (colegio_id, nivel);

create table public.visitas_programadas (
  id uuid primary key default gen_random_uuid(),
  colegio_id uuid not null references public.colegios(id),
  fecha timestamptz not null,
  tipo text not null,
  nivel text check (nivel in ('preescolar', 'primaria')),
  numero_clase integer check (numero_clase > 0),
  periodo smallint check (periodo between 1 and 4),
  serie_id uuid,
  ...
  estado text not null default 'programada' check (
    estado in ('programada', 'pendienteConfirmacion', 'confirmada',
               'realizada', 'cancelada', 'reprogramada')
  ),
  motivo_cancelacion text not null default '',
  ultima_novedad text not null default '',
  creado_en timestamptz not null default now()
);
create unique index visita_clase_unica
  on public.visitas_programadas (colegio_id, nivel, numero_clase)
  where numero_clase is not null and estado <> 'cancelada';
create index visitas_colegio_fecha
  on public.visitas_programadas (colegio_id, fecha);

create table public.visitas_acompanantes (
  visita_id uuid not null references public.visitas_programadas(id)
    on delete cascade,
  perfil_id uuid not null references public.perfiles(id),
  primary key (visita_id, perfil_id)
);

create table public.fechas_bloqueadas (
  fecha date primary key,
  motivo text not null default '',
  bloqueada_por uuid references public.perfiles(id),
...
create table public.evaluaciones_capacitacion (
  ...
  colegio_id uuid not null references public.colegios(id),
  nivel text not null check (nivel in ('preescolar', 'primaria')),
  evaluador_tipo text not null,
  responsable_id uuid references public.perfiles(id),
  estado text not null default 'borrador' check (
    estado in ('borrador', 'enProgreso', 'completada')
  ),
  fecha_creacion timestamptz not null default now(),
  reemplazos jsonb not null default '[]'::jsonb
    check (jsonb_typeof(reemplazos) = 'array')
);
create index evaluaciones_capacitacion_colegio
  on public.evaluaciones_capacitacion (colegio_id, nivel);

create table public.clases_capacitacion (
  id uuid primary key default gen_random_uuid(),
  evaluacion_id uuid not null references public.evaluaciones_capacitacion(id)
    on delete cascade,
  visita_id uuid references public.visitas_programadas(id),
  numero integer not null check (numero > 0),
  fecha timestamptz,
  bloques jsonb not null default '[]'::jsonb
    check (jsonb_typeof(bloques) = 'array'),
  bloque_canciones_seleccionado text,
  observaciones text not null default '',
  firma_docente_ruta text,
  firmas_asistentes_rutas jsonb not null default '[]'::jsonb
    check (jsonb_typeof(firmas_asistentes_rutas) = 'array'),
  unique (evaluacion_id, numero)
);

-- Una fila por docente y clase. Ausente equivale a "sin marcar".
create table public.asistencias (
  clase_id uuid not null references public.clases_capacitacion(id)
    on delete cascade,
  asignacion_id uuid not null references public.asignaciones_docentes(id),
  asistio boolean not null,
  registrado_en timestamptz not null default now(),
  registrado_por uuid references public.perfiles(id),
  primary key (clase_id, asignacion_id)
);
create index asistencias_asignacion
  on public.asistencias (asignacion_id);

create table public.reportes_periodo (
  id uuid primary key default gen_random_uuid(),
  colegio_id uuid not null references public.colegios(id),
  asignacion_id uuid references public.asignaciones_docentes(id),
  grado text not null,
  periodo smallint not null check (periodo between 1 and 4),
  fecha_hora timestamptz not null,
  nota numeric(5,2) check (nota >= 0),
  puntaje_maximo numeric(5,2) not null default 5 check (puntaje_maximo > 0),
  datos jsonb not null default '{}'::jsonb
    check (jsonb_typeof(datos) = 'object'),
  firma_colegio_ruta text,
  firma_docente_colegio_ruta text,
  firma_course_child_ruta text,
  evidencia_rutas text[] not null default '{}',
  aprobado_por uuid references public.perfiles(id),
  aprobado_en timestamptz,
  creado_en timestamptz not null default now(),
  check (nota is null or nota <= puntaje_maximo)
);
create index reportes_periodo_colegio_grado
  on public.reportes_periodo (colegio_id, grado, periodo);
create index reportes_periodo_asignacion
  on public.reportes_periodo (asignacion_id);

-- Fila única para los mínimos; solo el administrador podrá editarla.
create table public.configuracion_academica (
  id boolean primary key default true check (id),
  asistencia_minima_profesor smallint not null default 80
    check (asistencia_minima_profesor between 0 and 100),
  evaluacion_periodos_minima_profesor smallint not null default 75
    check (evaluacion_periodos_minima_profesor between 0 and 100),
  notas jsonb not null default '{}'::jsonb
    check (jsonb_typeof(notas) = 'object'),
  actualizado_en timestamptz not null default now(),
  actualizado_por uuid references public.perfiles(id)
);
insert into public.configuracion_academica (id) values (true);

alter table public.perfiles enable row level security;
alter table public.colegios enable row level security;
alter table public.docentes_colegio enable row level security;
...
alter table public.visitas_acompanantes enable row level security;
...
alter table public.clases_capacitacion enable row level security;
alter table public.asistencias enable row level security;
alter table public.reportes_periodo enable row level security;
alter table public.configuracion_academica enable row level security;

-- Los proyectos Supabase pueden conceder privilegios por defecto a los
-- clientes. Se revocan hasta definir las políticas y operaciones concretas.
revoke all on table public.perfiles from anon, authenticated;
revoke all on table public.colegios from anon, authenticated;
revoke all on table public.docentes_colegio from anon, authenticated;
revoke all on table public.asignaciones_docentes from anon, authenticated;
revoke all on table public.visitas_programadas from anon, authenticated;
revoke all on table public.visitas_acompanantes from anon, authenticated;
revoke all on table public.fechas_bloqueadas from anon, authenticated;
revoke all on table public.evaluaciones_capacitacion from anon, authenticated;
revoke all on table public.clases_capacitacion from anon, authenticated;
revoke all on table public.asistencias from anon, authenticated;
revoke all on table public.reportes_periodo from anon, authenticated;
revoke all on table public.configuracion_academica from anon, authenticated;
